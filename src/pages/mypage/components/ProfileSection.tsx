...
const ProfileSection: React.FC<ProfileSectionProps> = ({
  ...
  return (
    <div className="w-[360px] h-[108px] flex items-center justify-between px-[20px]" onClick={() => nav("/edit")}> 
      {/* 왼쪽 프로필 리스트 전체 */}
      <div className="flex items-center w-[296px] h-[84px] gap-[20px]">
        ...
        <div className="flex flex-col w-[192px] h-[47px] gap-2 justify-center">
          ...
          <span className="w-[192px] h-[16px] body-14-r text-gray-500">{gender} · {birthday}</span>
        </div>
      </div>
      {/* chevron_right 아이콘 */}
      <div className="flex items-center justify-center w-[24px] h-[24px]">
        <RightIcon className="w-6 h-12 text-gray-500" />
      </div>
    </div>
  );
};
